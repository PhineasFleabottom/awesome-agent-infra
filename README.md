...
| AgentMemory | Storage | ❌ **Dead & suspect.** All domains (.dev/.com/.io/.ai) parked or unresolvable. Listed on ClawHub as "100GB free E2E encrypted agent memory" but nothing was built. Even if it had launched: sending agent context (which contains credentials, personal data, API keys) to an unverified third party is a credential-harvesting risk. No Moltbook users have confirmed it works. |
